...
function Page(){
	...
	this.init= function(){
        ...
        $("[data-toggle=popover]").popover({ trigger:"manual" }).click(function() {
            var pop = $(this);
            pop.popover("show");
        ...
        $(document).on('click', '.btnActionUpdate', function(){
            ...
            _data     = _data + '&' + $.param({token:token,nowtimezone:timezone_offset_minutes});
                ...
                $.post(_action, _data, function(result){
                    ...
                    if(result['st'] == "success"){
                        window.location.assign(_redirect);
                            ...
                            $(".input-group").last().find('.alert-danger').remove();
        ...
        $(document).on('click', '.btnActivityAll', function(){
            ...
            _payments = _that.parents(parentSelector).data("payments");
            ...
            if(!_that.hasClass('disabled')){
                ...
                $.post(_action, _data, function(result){
                    ...
                    if(result['st'] == "success"){
                        ...
                        //self.showNotification(result['label'], result['txt'], 'bottom', 'center', 'animated bounceIn', 'animated bounceOut');
                        //swal("Warning!", result['txt'], "warning");
                        swal({
                            title: 'warning!',
                            text: result['txt'],
                            type: 'warning',
                            ...
                            confirmButtonText: 'CLICK HERE TO START YOUR SESSION'
                        },function () {
                            //alert("ppp");

                            window.location.href = "https://app.igplan.com/index.php/payments";
            ...
            if(!_that.hasClass('disable')){
                ...
                $.post(PATH+"search/search_tag", _data, function(result){
                    if(result['st'] == 'success'){
                        ...
                        self.delLoading();
                        var inner = '<div role="alert" class="bootstrap-alert bootstrap-alert-danger error-message"><strong>'+ result['txt'] +'</strong></div>';
                        $('#home').prepend(inner);
                        //self.showNotification(result['label'], result['txt'], 'bottom', 'center', 'animated bounceIn', 'animated bounceOut');
                    }
                    _that.removeClass('disable');
        ...
        $(document).on("change", ":input[name='enable_followers']", function(){
                ...
                $.post(_action, _data, function(result){
                    if(result.st == 'newerror'){
//alert(121);

                        self.showAlertMessage(result.txt, function(){
                            var newid = $("#newid").text();
                            _that    = $('.btnOpenAddUsernames');
        ...
        $(document).on("change", ":input[name='todo_like']", function(){
            ...
            $.post(_action, _data, function(result){
                ...
                }else if(result.st == 'newerror'){
                    self.showAlertMessage(result.txt);
                }else{
        ...
        $(document).on("click", ".PackMail", function(){
            ...
            $.post(_action, _data, function(result){
                ...
                if(result.st == 'error'){
                    ...
                    self.showSuccessMessage(result.txt, function(){
                        window.location.reload();
                    });
                    //alert(879);
                }
        ...
        $(document).on('click', '.btnAddEmail', function(){
            _that     = $(this);
            //_form     = _that.closest("form");
            _action   = PATH+"search/ajax_add_email";
            //_type     = _form.data('type');
            //_data     = _form.serialize();
            _account = $(":input[name='account']").val();
            _email = $(":input[name='email']").val();
            _data = $.param({token:token,id:_account ,email:_email});
            //_data     = _data + '&' + $.param({token:token, type: _type});
            ...
            $.post(_action, _data, function(result){
                if(result.st == 'success'){
                    ...
                    //$("#modal-add-email").modal('hide');
                    self.showSuccessAutoClose(result.txt, "error", 2000);
                    //self.showNotification(result.label, 'Deleted!', 'bottom', 'center', 'animated bounceIn', 'animated bounceOut');
        ...
        $(document).on('click', '.btnDeleteEmail', function(){
            _that     = $(this);
            //_form     = _that.closest("form");
            _action   = PATH+"search/ajax_delete_email";
            //_type     = _form.data('type');
            //_data     = _form.serialize();
            _account = $(":input[name='account']").val();
            _email = $(":input[name='email']").val();
            ...
            if(!_form.hasClass('disable')){
                ...
                $.ajax({
                        ...
                        if(result['st'] == "success"){
                            ...
                            if(result.hasOwnProperty('accountStatus') && result['accountStatus'] == true) {
                                ...
                                if(_redirect == ''){
                                    ...
                                    $('#new-name').text($('.instagram-username').text());

                                    $('.successfully-reconnected').removeAttr('hidden');

                                }

                                if(result.hasOwnProperty('newaccount') && result['newaccount'] == true) {
                                    $('#getstartguide').modal({ backdrop:'static',keyboard:true,show:true });
                    ...
                    error: function (result) {
                        ...
                        $('.error-message').empty().append('There is a problem in sending verification code to Mobile/Email. Please go to instagram verify yourself manually and try it again.');
            ...
            if(!_form.hasClass('disable')){
                ...
                $.post(_action, _data, function(result){
                    ...
                    if(result.st == 'modal'){
                        ...
                        $("body").append(result.txt);

                        $('#modal-reconnect').modal('show');

                    }else if(result.st == 'valid'){
                        //self.showNotification(result.label, result.txt, 'bottom', 'center', 'animated bounceIn', 'animated bounceOut');
                        //alert('if');
                        if(newstatus == 3) {
    ...
    this.showConfirmMessage = function($message, $function) {
        swal({
            ...
            confirmButtonColor: "#1471b9",
            confirmButtonText: Lang["yes"],
            closeOnConfirm: false
        }, $function);
    }

    this.showAlertMessage = function($message, $function) {
        swal({
            title: $message,
            type: "warning",
            showCancelButton: false,
    ...
    this.showSuccessMessage = function($message, $function) {
        swal({
            title: $message,
            type: "success",
            showCancelButton: false,
