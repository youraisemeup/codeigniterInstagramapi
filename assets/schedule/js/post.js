...
//alert(type);
    ...
    filemanager.setOption("onBeforeUpload", function() {
        if ($(window).width() <= 600) {
            $(".mobile-uploader-result").html("Uploading...").stop().fadeIn();
        }
    });
    ...
    $form.on("submit", function() {
        ...
        data.post = $form.find(":input[name='post']").val();
        ...
        if (data.type == "album" && media_ids.length < 2) {
            NextPost.DisplayFormResult($form, "error", "Please select at least 2 media album post.");
            return false;
        } else if (data.type == "story" && media_ids.length != 1) {
            NextPost.DisplayFormResult($form, "error", "Please select one media for story post.");
            return false;
        } else if (data.type == "timeline" && media_ids.length != 1) {
            NextPost.DisplayFormResult($form, "error", "Please select one media for post.");
            return false;
        } else if (!data.accounts || data.accounts.length < 1) {
            NextPost.DisplayFormResult($form, "error", "Please select at least one Instagram account.");
            return false;
        }
        ...
        $.ajax({
            ...
            data: data,
            error: function() {
              //  NextPost.DisplayFormResult($form, "error", "Oops! An error occured. Please try again later!");
            ...
            success: function(resp) {
                if (typeof resp.result == undefined){
                    ...
                    var msg = resp.msg || "Oops! An error occured. Please try again later!";
    ...
    $("#savetag").on("click",function(){
        ...
        var path = $('#hashpath').text();

        var data = {};

        data.caption = emoji[1].emojioneArea.getText();
        data.action = "save";


        data.id = $('#captionid').val();
...
//        alert('hello');
            ...
            $.ajax({
                ...
                success: function(data){
                    $('.commentbox').find('.emojionearea-editor').html(data.caption);
//                    $('.commentbox').text(data.msg[0].caption);
                    $('#captionid').val(data.id);
                }
            });
...
//        ctx.drawImage(video, 0, 0, 160, 90);
        var image_base_64 = canvas.toDataURL();
        //alert(canvas.toDataURL());
        //thumbs.appendChild(canvas);
        //alert(image_base_64);
        // var pathname = window.location.pathname; // Returns path only
        //var url      = window.location.href;
        $('.fluid.large.button').attr('disabled',true);
        //$('.fluid.large.button').css('background-color','#f5f7f8');
        $('.fluid.large.button').css({"background-color":"#f5f7f8","border": "1px solid #f5f7f8","color": "#000"});

        $.ajax({
            url: 'https://app.igplan.com/index.php/post/imageupload',
            //url: url,
            type: 'POST',
            data: { action: 'imageupload',ImageName: src, Image : image_base_64} ,
            //contentType: 'application/json; charset=utf-8',
            success: function (response) {
                //alert(response);
                //return response;
                $('.fluid.large.button').removeAttr('disabled');
                //$('.fluid.large.button').css('background-color','#0D509F');
                $('.fluid.large.button').css({"background-color":"#0D509F","border": "1px solid #0D509F","color": "#fff"});
            },
            error: function () {
                $('.fluid.large.button').removeAttr('disabled');
                //$('.fluid.large.button').css('background-color','#0D509F');
