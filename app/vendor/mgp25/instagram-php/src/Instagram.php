...
class Instagram implements ExperimentsInterface
{
	...
	// Custom Functions By Jammi For Code Verification
	
	
    public function sendChallengeVerificationCode($username,
                                    $password,$url)
    {
        ...
        if ($this->username !== $username || $this->password !== $password) {
            ...
            try {
                $response = $this->request('https://i.instagram.com/api/v1'.$url)
	 ...
	 public function sendChallengeVerificationCodeToMObile($username,
                                    $password,$url)
    {
    ...
    public function approveChallengeVerificationCode(
        $instagram_id,
        $challenge_id,
        $security_code,$username,$password)
    {
        if ($this->username !== $username || $this->password !== $password) {
            $this->_setUser($username, $password);
        }
