...
//        $videoDetails = [];
        $streamInfo = array('codec_type'=>'video');
...
//                    throw new \InvalidArgumentException(sprintf(
//                        'Instagram only accepts story videos that are between 3 and 20 seconds long. Your story video "%s" is %.3f seconds long.',
...
////                if ($aspectRatio < 0.56 || $aspectRatio > 0.67) {
//                    throw new \InvalidArgumentException(sprintf(
//                        'Instagram only accepts story media with aspect ratios between %.3f and %.3f. Your file "%s" has a %.4f aspect ratio.',
//                        0.56, 0.67, $mediaFilename, $aspectRatio
//                    ));
//
////                    throw new \InvalidArgumentException(sprintf(
////                        'Instagram only accepts story media with aspect ratios between %.3f and %.3f. Your file "%s" has a %.4f aspect ratio.',
////                        MIN_STORY_RATIO, MAX_STORY_RATIO, $mediaFilename, $aspectRatio
////                    ));
//                }
                break;
            default:
...
////                    throw new \InvalidArgumentException(sprintf(
////                        'Instagram only accepts media with aspect ratios between %.3f and %.3f. Your file "%s" has a %.4f aspect ratio.',
////                        0.8, 1.91, $mediaFilename, $aspectRatio
////                    ));
//                }
        }
    }
        ...
        $videoFilename)
    {
        ...
        $ImageFileName= str_replace($ext,"png",$videoFilename);
        // die();
...
//        $ffmpeg = self::checkFFMPEG();
        ...
        //$userid  =  $this->extract_unit($videoFilename,'uploads','/');

//        $new_vdo_path =explode('.',str_replace("2/","temp/",$videoFilename));

//$ImageFileName=$new_vdo_path[0].'.png';
...
//        $tmpFilename = $tmpPath.'/kovazoye-5ae04377596c1.jpg';
        $tmpFilename = $ImageFileName;
//        $tmpFilename ='https://blog.majestic.com/wp-content/uploads/2010/10/Video-Icon-crop.png';
...
//            $jpegContents = file_get_contents('https://blog.majestic.com/wp-content/uploads/2010/10/Video-Icon-crop.png'); // Process&get.
            $resizer->deleteFile();
